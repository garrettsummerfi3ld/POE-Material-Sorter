...
task init() {
	...
	// Set called containter to '1'
	writeDebugStreamLine("[INIT] [VAR] Setting 'calledStorageContainer' variable to '1'...");
	calledStorageContainer = 1;
	writeDebugStreamLine("[INIT] [VAR] 'calledStorageContainer' var set!");

	// Clear timer
	writeDebugStreamLine("[INIT] [VAR] Clearing timer 'T1'...");
	clearTimer(T1);
	writeDebugStreamLine("[INIT] [VAR] Timer cleared!");
...
bool checkAround(int value, int goal) {
	...
	if (value > 1000) {
		if (goal - 500 < value && value < goal + 500) {
			writeDebugStreamLine("[CHECK] Goal met!");
			return true;
		} else {
			writeDebugStreamLine("[CHECK] Goal not met!");
			return false;
		}
	} else {
...
task detectMaterial() {
	...
	if(lightVal > 30 || lineVal < 2960) {
		...
		// Neutral
		if (checkAround(lightVal, 50) && checkAround(lineVal, 2660)){
			writeDebugStreamLine("none");
		// Clear detection process
		if(checkAround(lightVal, 20) && checkAround(lineVal, 2890)) {
		...
		if(checkAround(lightVal, 200) && checkAround(lineVal, 300)) {
		...
		if(checkAround(lightVal, 310) && checkAround(lineVal, 1530)) {
		...
		if(checkAround(lightVal, 310) && checkAround(lineVal, 2960)) {
...
task main() {
	...
	while(true) {
		...
		if(callReset == true) {
			...
			writeDebugStreamLine("Starting 'detectMaterial' task...");
			startTask(detectMaterial);
			writeDebugStreamLine("Starting 'checkListen' task...");
